...
/** 验证表格勾选项的必填项是否都有值 */
export function validateTableSelected(tableColumn: TableColumn[], selectedList: any[]) {
  let flag = true
  let message = ''
 ...
 */
export function checkItemToTop(originArr:any[], targetItem:any):any[] {
  let _arr:any[] = []
  ...
  if (Array.isArray(targetItem)) {
    ...
    // 先排除目标项
    _arr = deepClone(originArr.filter(item => item.uuid !== targetItem.uuid)) as any[]
    // 然后将目标项从头插入即可实现置顶
  }
  if (Array.isArray(targetItem)) {
