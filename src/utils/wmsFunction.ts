 ...
 * @returns
 */
const getOutStockParams = (_list: any[], form: any, userObj: any, imOclas: string, wmsOclas?: string) => {
  ...
  _list.forEach((item) => {
    let obj = {
      ...
      barcode: item.newBarcode ?? item.barcode,
      stationID: item.stationNo,
      batchID: item.batch,
      oclas: wmsOclas ?? item.oclas, // 移到类型
 ...
 * @returns
 */
const getPostInParams = (_list: any[], form: any, userObj: any, imOclas: string, wmsOclas?: string) => {
 ...
 * @returns
 */
const getEditableKey = (tableColumn: any[]) => {
 ...
 *  @param tableColumn:表格项数据
 */
const computedTotal = (arr: any[], tableColumn: any[], keywords?: string) => {
