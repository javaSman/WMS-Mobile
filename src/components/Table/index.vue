    ...
    <div class="wrapper" ref="wrapperRef" @scroll="onScroll">
      ...
      <div class="list" ref="container">
        <van-form ref="refForm">
          <van-checkbox-group v-model="virtualSelected" ref="checkboxGroup" @change="handleSelect">
              ...
              >
                <template #input>
                  ...
                  <div v-else-if="item.type === 'Table/Input'" style="width: 100%">
                    <van-field
                      :ref="(el: any) => tableInputRef[item.prop + '_' + index] = el"
                  ...
                  <div v-else-if="item.type === 'Table/NumberInput'" style="width: 100%">
                    ...
                    >
                      <template #extra v-if="item.tableNumberParams">
                        {{ data[(item.tableNumberParams as any).exProp] }}
                      </template>
                    </van-field>
                  </div>
                  <!-- 针对可以输入项自定义设计 -->
...
let tableInputRef = ref<any>({})

// 非虚拟滚动下，保持原本的
let selected = ref<string[]>([])

// 在虚拟滚动的情况下，直接利用实现双向绑定
const virtualSelected = computed({
  get() {
