...
let barcodeInputRef: FieldInstance | null = null // 条码ref
  ...
  GetBoxInfoByBoxId(val)
    .then((res: any) => {
      ...
      if (res.success) {
        // 判断箱子号是否可以带出货位
        if (res.locationNo) {
          form.value.locationID = res.locationNo as string // locationID 字段根据需求
...
const barcodeEnterByAddTo = async (val: string | number | null) => {
  ...
  try {
    ...
    let res: any = await WMSAPI.get(props.listAPIName, { barcodes: [val] })
    if (res && res.success) {
      let _data: any[] = res.barcodeList
      // 清空输入框
...
const handleConfirm = async () => {
  try {
    ...
    let _data = {}
    if (isPostIn.value) {
      _data = getPostInParams(_list, form.value, { account, name }, props.type, props.wmsOclas)
    } else {
      _data = getOutStockParams(_list, form.value, { account, name }, props.type, props.wmsOclas)
    }
    // 表单验证完后，若有自定义方法，则执行
    if (props.submit) {
...
function validateErfmg(): any {
  ...
  if (/^\+?[1-9][0-9]*$/.test(list.value[index][key])) {
    // 原始值
