...
import { getPostInParams, computedTotal } from '@/utils/wmsFunction'
...
function getBoxDetails(val: string | null) {
  ...
  GetBoxDetail(val)
    .then((res: any) => {
      if (res.success) {
        let boxObj: any = res.box ?? {}
        if (Object.getOwnPropertyNames(boxObj).length === 0) {
          _showFailToast(`箱子号[${val}]不存在，请重新扫码！`)
          return
        } else {
          form.value.locationID = boxObj.locationNo
        }
...
const enterByAddTo = (val: string | number | null, paramsProp: string, listProp: string, inputProp: string) => {
  ...
  if (stashList.value.length > 0) {
    let target = stashList.value.filter((item) => item[props.scanlKey] === val)
    if (target.length > 0) {
      _showFailToast('已存在载具内，请勿重复操作')
