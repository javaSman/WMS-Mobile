...
import { MOVE_TYPE, formatLgortConnector, formatLgottConnector, formatEbeln } from '@/views/dict'

let minDate = Date.now() - 1000 * 3600 * 24 * 30 * 6
export const formList: Array<FormConfig> = [
  { type: 'Text', label: '工号', prop: 'imCardno' },
  { type: 'Calendar', label: '过账日期', prop: 'imPostdate', calendarParams: { minDate: new Date(minDate)}, rules: [{ required: true, message: '请选择过账日期', trigger: 'onChange' }] },
  {
    type: 'Dropdown',
    label: '移动类型',
    prop: 'imMovetype',
...
// cpudt cputm
function formatCputm(data: any, val: string) {
