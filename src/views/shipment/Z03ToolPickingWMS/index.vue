<template>
  ...
  <van-sticky :offset-top="46">
    ...
    >
      ...
      <van-grid-item>
        <van-checkbox v-model="selectAll" :disabled="disableCheck" shape="square" @click="handleSelectAll">
          全选
        </van-checkbox>
      </van-grid-item>
    </van-grid>
...
import { getEditableKey, validateAuthAndSelected, validateQtyForItem } from '@/utils/validateOperateForQty'
...
const formateTableColumn: any[] = tableColumn.map((item) => {
  ...
  //   return { ...item, rules: [{ required: true, message: '请输入数量', trigger: 'onBlur' }, { validator: validateFunc }] }
  // } else {
  //   return item
  // }
})
  ...
  WMSAPI.get(listAPIName, {
    ...
    .then((res) => {
      if (res.success) {
        ...
        // 混入唯一值，用于勾选数据
        // tips: 这里要特殊处理一下barcode，需将字段“reqno”拼接“rspos”作为WMS的barcode
...
const handleConfirm = async () => {
  ...
  await validateQtyForItem(selection.value, key)
  showConfirmDialog({
    title: '提示',
