...
<script lang="ts" setup name="transfer.Z44CrossRegionalTransferPicking-WMS">
// Z44跨区域调拨拣货-WMS
...
function barcodeEnter(val: string | null) {
  ...
  if (dataMap.list.length > 0) {
    // 从结果集中寻找
    let target = dataMap.list.find((item) => item.reqno === val)
    if (target) {
      // 进行勾选
      dataMap.selectedList.push(target)
      nextTick(() => {
  ...
  WMSAPI.get(listAPIName, { imBarcode: val, imOclas })
    .then((res) => {
      if (res && res.success) {
        ...
        dataMap.list = arr
        // 全选
        table.value?.handleSelect(arr.map((item) => item.uuid))
      } else {
        _showFailToast(res.message as string)
      }
    })
...
async function handleConfirm() {
  try {
    ...
    if (dataMap.selectedList.length !== dataMap.list.length) {
