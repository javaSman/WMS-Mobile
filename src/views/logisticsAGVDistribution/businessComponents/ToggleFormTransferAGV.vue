  ...
  <div>
    ...
    <TableVue ref="table" v-model:table-data="list" :tableColumn="tableColumn" v-model:selection="selectedList">
      ...
      <template #locationId="inputProps">
        <van-field v-model="list[inputProps.index][inputProps.item.prop]" clearable clear-trigger="always" />
      </template>
    </TableVue>
    <!-- 底部按钮 -->
    <ActionBarVue
...
let drawerInputRef: FieldInstance | null = null
/** 仓库change */
...
function boxNoEnter(val: string | null, target: string) {
  ...
  WMSAPI.get('business/webapi/GetLocationNoByBoxId', { boxId: val })
    .then((res: any) => {
      // console.log(res)
      // 如果当前是转出箱子，则跳转到条码
...
// 储物格回车
function drawerEnter(val: string | null) {
  if (!val) return
  // 调用接口带出区域，如果储物格有值，光标跳转到条码，没有跳转到储物格
  form.value.agvNo = val.split('-')[0]
  WMSAPI.post('business/webapi/AGVOpenDrawer', {
    agvNo: form.value.agvNo,
    drawer: val
  })
    .then((res: any) => {
      if (res.success) {
 ...
 * @param {*} val 条码值
 */
const barcodeEnterByAddTo = async (val: string | number | null) => {
  if (!val) {
    _showFailToast('请输入条码')
    return
  }
  ...
  try {
    ...
    let params = {}
    if (props.type) {
      params.imOclas = props.type
    }
    if (props.codeType) {
      params[props.codeType] = val
    } else {
      params.imBarcode = val
    }
    let res = await WMSAPI.get(props.listAPIName, params)
    if (res && res.success) {
      if (!res.data) {
        res.drawer = form.value.drawer
        res.agvNo = form.value.agvNo
        res.imBarcode = form.value.imBarcode
        res.operator = account + ' - ' + name
      ...
      selectedList.value = JSON.parse(JSON.stringify(list.value))
      // 存储原始数据
...
const handleConfirm = async () => {
  try {
    ...
    await showConfirmDialog({
      ...
      message:
        props.confirmTips ||
        `本次过账共 ${selectedList.value.length} 条数据,累计数量是: ${computedTotal(selectedList.value)},是否确认过账？`
    })
    // 开始过账
    loading.value = true
    // 删除uuid,遍历实现
...
const getOutStockParams = (_list: any[]) => {
  let { reqno, locationID, targetBoxID } = form.value
...
async function initConfig() {
  ...
  _formList.value.forEach((item) => {
    ...
    if (item.prop === 'drawer') {
      item.enter = (val: string | null) => drawerEnter(val)
      drawerInputRef = formComponent.value?.formInputRef[item.prop].inputRef
    }
  })
}
